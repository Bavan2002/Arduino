...
ISR(TIMER2_COMPA_vect) {
    timer2_millis++;
    update_encoder(); // Call update_encoder() function during Timer2 interrupt
...
void update_encoder() {
    ...
    if (0 <= angle && angle <= 90) q = 1; // Update the quadrant
...
void updatePID() {
    if (moving) {
        ...
        doPID(&leftPID_Output, encoder1, targetPosition1, &left_prev_encoder, &left_prev_input, &left_ITerm);
        doPID(&rightPID_Output, encoder2, targetPosition2, &right_prev_encoder, &right_prev_input, &right_ITerm);
